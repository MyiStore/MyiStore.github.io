...
// тут тоже :)
function RemovingElFromBasketOrComprBasket(parentWithCancelBtns, visual_counter, LocalStorageBasketsUsed, BasketsUsed, typeOfFooterBlock, parent_block) {
...
function PushBasketOrCompInfInLocStor(localStorage_key, mobileOrPcVerBasketBlock, basketCounter, footer_visual_block, parent_block) {
    let getBasketPhonesInf = localStorage.getItem(localStorage_key);
    let inf_to_del = mobileOrPcVerBasketBlock.querySelector('.compation_text');
    inf_to_del.innerText = '';
    let parsedBasketPhonesInf = JSON.parse(getBasketPhonesInf);
...
function ChangingTotalPriceForBasket(getPrice) {
...
function RemovingElFromBasketIndependently(show_products_modal_block, textInfBlock, svgChangeColor, className, visualCounter, footerTotalElement) {
